...
function RpcClient(opts) {
  ...
  this.port = opts.port || 3889;
  this.user = opts.user || 'exploreradmin';
  this.pass = opts.pass || 'explorer';
...
function rpc(request, callback) {
  ...
  var errorMessage = 'SBER JSON-RPC: ';
  ...
  var req = this.protocol.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
        var exceededError = new Error('SBER JSON-RPC: ' + buf.toString('utf8'));
